...
MBButton::MBButton(string btn_label, string cur_level, double b_x, double b_y, string img, double b_w, double b_h, bool murph) :
    ...
    m_x(b_x), m_y(b_y), m_h(b_h), m_w(b_w), m_font_size(-1), m_active(true), m_blackhole(murph), m_font_color(Color::WHITE) {
...
MBButton::MBButton(string btn_text, string btn_label, string cur_level, double b_x, double b_y, double b_w, double b_h, int font_size, const Color& color, bool murph) :
    m_click_state(NOT_CLICKING), m_hover_state(NOT_HOVERING), m_label(btn_label), m_level(cur_level),
    m_x(b_x), m_y(b_y), m_h(b_h), m_w(b_w), m_font_size(font_size), m_active(true), m_active_texture(false), m_blackhole(murph), m_font_color(color) {
...
MBButton::MBButton(string btn_text, string btn_label, string cur_level, string img, double b_x, double b_y, double b_w, double b_h, int font_size, bool murph) :
    m_click_state(NOT_CLICKING), m_hover_state(NOT_HOVERING), m_label(btn_label), m_img(img), m_level(cur_level),
    m_x(b_x), m_y(b_y), m_h(b_h), m_w(b_w), m_font_size(font_size), m_active(true), m_active_texture(false), m_blackhole(murph), m_font_color(Color::WHITE) {
...
bool MBButton::on_event(const GameEvent& event){
    ...
    else if(event.id() == GAME_MOUSE_RELEASED){
        double mouse_x = event.get_property<double>("x");
        double mouse_y = event.get_property<double>("y");
...
void MBButton::draw_self(Canvas *canvas, unsigned, unsigned){
    if(m_active){
        ...
        if(not m_text.empty()){
            ...
            canvas->draw(m_text, m_x + m_w/2 - len/2, m_y + font_size / 2 - 5);
        }
    }
}
